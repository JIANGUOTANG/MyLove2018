package com.mylove2018.calendard;
...
public class CalendarSelector {
    ...
    public interface ICalendarSelectorCallBack {
        void transmitPeriod(HashMap<String, String> result, boolean isLunar);
    }
    ...
    private void initListener() {
        tvConfirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (isLunar) {
                    //同时返回新历的内容，用于计算两个日期的天数
                    HashMap<String, Object> map2 = Utils.lunar2Average(mYearPicker.getSelectedItem(),
                            mMonthPosition, mDayPosition);
                    mYearPosition = (int) map2.get("yearPosition");
                    mMonthPosition = (int) map2.get("monthPosition");
                    mDayPosition = (int) map2.get("dayPosition");
                    mMap = Utils.parseAverageYear(mYearPicker.getSelectedItem());
                    mMonthList = (List<String>) mMap.get("month");
                    mDayList = ((List<List<String>>) mMap.get("day")).get(mMonthPosition);
                    map.put("year2", mYearList.get(mYearPosition));
                    map.put("month2", mMonthList.get(mMonthPosition));
                    map.put("day2", mDayList.get(mDayPosition));
                }
                mCallBack.transmitPeriod(map, isLunar);
